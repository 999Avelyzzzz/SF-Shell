...
static const char *DEFAULT_CSS =
    ...
    "}\n"
    "\n"
    "/* Contenitore orologio: stessa pill della batteria, con bolla in hover. */\n"
    ".pill {\n"
    "  border-radius: 7px;\n"
    "  padding: 2px 7px;\n"
    ...
    "  color: @theme_fg_color;\n"
    "}\n"
    "/* Icone della tray: dissolvenza sincronizzata allo slide del revealer.\n"
    "   .collapsed (togglata dal codice) porta l'opacita' a 0 in chiusura. */\n"
    ".tray-icons {\n"
    "  opacity: 1;\n"
    "  transition: opacity 200ms ease;\n"
    "}\n"
    ".tray-icons.collapsed {\n"
    "  opacity: 0;\n"
    "}\n"
    ...
    "}\n"
    "\n"
    "/* ---- Batteria (stile macOS: \"37%\" + glifo) ---- */\n"
    "/* La pill: nessuna cromatura di default, forma a pastiglia. Hover -> bolla\n"
    "   bianca semitrasparente dietro numero e glifo; checked (popover aperto)\n"
    "   -> bolla bianca piena. Transizione morbida. */\n"
    ".battery {\n"
    "  all: unset;\n"
    "  border-radius: 7px;\n"
    "  padding: 2px 7px;\n"
    ...
    "}\n"
    "/* Numero: stesso font della barra, cifre a larghezza fissa (tnum). */\n"
    ".battery-pct {\n"
    "  color: @theme_fg_color;\n"
    "  font-size: 12px;\n"
    "  font-weight: 500;\n"
    "  font-feature-settings: \"tnum\";\n"
    "}\n"
    "/* Glifo di barra: colore letto in cairo (contorno/polo/riempimento). */\n"
    ".battery-glyph {\n"
    "  color: @theme_fg_color;\n"
    "}\n"
    "/* Pill attiva: su sfondo bianco, numero e glifo diventano neri. */\n"
    ".battery:checked .battery-pct,\n"
    ".battery:checked .battery-glyph {\n"
    "  color: #000000;\n"
    "}\n"
    "\n"
    "/* Popover batteria: pannello scuro semitrasparente, angoli tondi, ombra. */\n"
    "popover.battery-menu {\n"
    "  background: transparent;\n"
    "}\n"
    "popover.battery-menu > contents {\n"
    "  background-color: alpha(@theme_bg_color, 0.72);\n"
    "  color: @theme_fg_color;\n"
    "  border: 1px solid alpha(@theme_fg_color, 0.10);\n"
    "  border-radius: 14px;\n"
    ...
    "}\n"
    "/* Glifo grande nel popover: sempre chiaro (sfondo scuro). */\n"
    ".battery-pop-glyph {\n"
    "  color: @theme_fg_color;\n"
    "}\n"
    ".battery-menu-pct {\n"
    ...
    "  font-feature-settings: \"tnum\";\n"
    "}\n"
    "/* Colori condizione (system colors macOS). */\n"
    ".battery-cond-good { color: #34c759; }\n"
    ".battery-cond-fair { color: #ffcc00; }\n"
    ".battery-cond-poor { color: #ff453a; }\n"
    "\n"
    "/* ---- App launcher ---- */\n"
    ...
    "}\n"
    "\n"
    "/* Velo scuro sopra il wallpaper sfocato: da' profondita' e leggibilita'.\n"
    "   Copre tutto il backdrop, sotto al pannello. */\n"
    ".launcher-tint {\n"
    "  background-color: rgba(0, 0, 0, 0.32);\n"
    "}\n"
    "\n"
    "/* Pannello: scala dal centro mantenendo l'aspect ratio (scale uniforme),\n"
    "   veloce e smooth (easeOutCubic). Semitrasparente per lasciar trasparire\n"
    "   il wallpaper sfocato dietro (effetto vetro smerigliato). */\n"
    ".launcher-panel {\n"
...
int main(int argc, char **argv)
{
    ...
    /* Prima di GTK/Pango: restringe fontconfig ai soli font locali. Serve
     * solo al processo che disegna la shell (comando "run"): l'invocazione
     * di "launcher" e' un client che inoltra il comando via D-Bus all'istanza
     * gia' in esecuzione e poi esce senza disegnare nulla, quindi non ha
     * bisogno dei font. Saltare la scansione fontconfig qui elimina un ritardo
     * fisso tra `sfshell launcher` e la comparsa del launcher. */
    if (strcmp(cmd, "run") == 0)
        fonts_init();
